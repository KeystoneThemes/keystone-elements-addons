...
class Widget_KEA_Search_Form extends Widget_Base {

	public function get_name() {
		return 'cj-search_form';
	}

	public function get_title() {
		return "[KS] " . esc_html__( 'Search Form', 'keystone-elements-addons' );
	}
	...
	public function get_script_depends() {
		return [ 'jquery-ui-autocomplete', 'cj-search_form' ];
    }
	...
	protected function register_controls() {
		...
		$this->add_control(
			'placeholder', [
				...
				'default' => esc_html__( 'Enter a Keyword...', 'keystone-elements-addons' ),
				'label_block' => true,
			]
		);
		...
		$this->add_responsive_control(
			'thumb_size',
			[
				'label' => esc_html__( 'Thumbnail Size (px)', 'keystone-elements-addons' ),
				'type' => \Elementor\Controls_Manager::NUMBER,
				'min' => 0,
				'max' => 150,
				'step' => 1,
				'default' => 40,
				'selectors' => [
					'{{WRAPPER}} .kea-search-form-container .kea-search-form .ui-menu .ui-menu-item .kea-search-ui-img img' => 'width: {{VALUE}}px;'
				],
			]
		);
		...
		$this->add_control(
			'thumb_border_radius',
			[
				'label' => esc_html__( 'Thumbnail Border Radius', 'keystone-elements-addons' ),
				'type' => Controls_Manager::DIMENSIONS,
                'size_units' => [ 'px', '%', 'rem' ],
				'selectors' => [
					'{{WRAPPER}} .kea-search-form-container .kea-search-form .ui-menu .ui-menu-item .kea-search-ui-img img' => 'border-top-left-radius: {{TOP}}{{UNIT}};border-top-right-radius: {{RIGHT}}{{UNIT}};border-bottom-right-radius: {{BOTTOM}}{{UNIT}};border-bottom-left-radius: {{LEFT}}{{UNIT}};'
				]
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%', 'rem' ],
				'selectors' => [
                    '{{WRAPPER}} .kea-search-form-container .kea-search-form .ui-menu .ui-menu-item .kea-search-ui-img img' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};'
				],
			]
		);
    ...
    protected function render() {
        ...
        <div class="kea-search-form-container <?php if (is_rtl()) { echo 'form-is-rtl'; } ?>" data-posttype="<?php echo esc_attr($settings['post_type']); ?>" data-smax="<?php echo esc_attr($settings['max']); ?>" data-smin="<?php echo esc_attr($settings['min']); ?>"  data-sdelay="<?php echo esc_attr($settings['delay']); ?>">
			<form id="kea-search-form-<?php echo $widget_id; ?>" role="search" method="get" class="kea-search-form ui-front" style="display:flex;" action="<?php echo esc_url(home_url( '/' )); ?>">
				<input type="text" class="kea-search-form-term autocomplete-<?php echo $settings['autocomplete']; ?><?php if (is_rtl()) { echo '-rtl'; } ?>" placeholder="<?php echo esc_attr($settings['placeholder']); ?>" name="s" />
		...
		if ($settings['autocomplete']) {
			wp_localize_script( 'cj-search_form', 'keaAutocomplete', array( 'url' => admin_url( 'admin-ajax.php' )) ); 
		}
		?>
	<?php }
}
Plugin::instance()->widgets_manager->register( new Widget_KEA_Search_Form() );