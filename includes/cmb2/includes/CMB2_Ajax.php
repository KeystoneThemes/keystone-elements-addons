...
class CMB2_Ajax {
	...
	public function get_oembed_no_edit( $args ) {
		...
		if ( isset( $args['object_type'] ) && 'post' != $args['object_type'] ) {
			...
			// Gets ombed cache from our object's meta (vs poscjta).
			add_filter( 'get_post_metadata', array( $this, 'hijack_oembed_cache_get' ), 10, 3 );

			// Sets ombed cache in our object's meta (vs poscjta).
	 ...
	 * Returns cached data from relevant object metadata (vs poscjta)
	 *
	 * @since  0.9.5
	 * @param  boolean $check     Whether to retrieve poscjta or override.
	 ...
	 * Saves cached data to relevant object metadata (vs poscjta)
	 *
	 * @since  0.9.5
	 * @param  boolean $check      Whether to continue setting poscjta.
	 * @param  int     $object_id  Object ID to get poscjta from.
	 * @param  string  $meta_key   Poscjta's key.
	 * @param  mixed   $meta_value Value of the poscjta to be saved.
	...
	public function hijack_oembed_cache_set( $check, $object_id, $meta_key, $meta_value ) {
		...
		// Anything other than `null` to cancel saving to poscjta.
		return true;
	}

	/**
	 * Gets/updates the cached oEmbed value from/to relevant object metadata (vs poscjta).
	 *
	 * @since 1.3.0
	 *
	 * @param string $meta_key Poscjta's key.
